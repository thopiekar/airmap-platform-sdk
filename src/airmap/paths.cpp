...
airmap::platform::Path airmap::paths::cache_dir(Client::Version version) {
  auto p = airmap::platform::create_interface();
  switch (version) {
    case Client::Version::production:
      return p->standard_paths()
                 .path(platform::StandardPaths::Scope::user, platform::StandardPaths::Location::cache)
                 .get() /
             "airmap" / "production";
    case Client::Version::staging:
      return p->standard_paths()
...
airmap::platform::Path airmap::paths::token_file(Client::Version version) {
  return cache_dir(version) / "token.json";
}
